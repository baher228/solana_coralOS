...
const isJobParty = (j, s) => Boolean(j && s && (j.employer === s.organization || j.worker === s.organization))
const canSubmitWork = (j, s) => canWork(j) && s?.role === 'worker' && j.worker === s.organization
const canReviewWork = (j, s) => canWork(j) && s?.role === 'employer' && j.employer === s.organization && Boolean(j.submission)
const activeDispute = (j) => (j?.disputes || []).find((d) => d.status === 'open')
...
function Delivery({ job, act, session }) {
  ...
  const canSubmit = canSubmitWork(job, session)
    ...
    <section class="panel">
      ...
      <div class="ms-board">
        ${job.milestones.map((m) => html`<div key=${m.id} class=${`ms-card ${m.status}`}>
          ...
          <button class="btn ghost sm" disabled=${!canSubmit || m.status === 'complete'} onClick=${() => act(() => api(`/api/jobs/${job.id}/milestones/${m.id}/complete`, { actor: 'worker' }), 'Milestone marked done')}>
      ...
      <form class="submit-form" onSubmit=${(e) => { e.preventDefault(); act(() => api(`/api/jobs/${job.id}/submission`, sub), 'Evidence submitted for review') }}>
        ...
        <button class="btn primary" disabled=${!canSubmit}><${Icon} name="box" /> Submit for review</button>
      </form>
    </section>
    <${DetailPanel} job=${job} session=${session} />
  </div></div>`
...
function Review({ job, act, session }) {
  ...
  if (isOpen(job)) return html`<div class="view"><div class="workspace">
    ...
    <${DetailPanel} job=${job} session=${session} /></div></div>`
  const canReview = canReviewWork(job, session)
  const releaseEligible = Boolean(job.review?.releaseEligible)
  const canRelease = canReview && releaseEligible && !activeDispute(job)
  const canDispute = isJobParty(job, session) && !isTerminal(job) && !isOpen(job)
  ...
  return html`<div class="view"><div class="workspace">
    <section class="panel">
      ...
      <div class="action-bar">
        ...
        <button class="btn danger" disabled=${!canDispute} onClick=${() => act(() => api(`/api/jobs/${job.id}/dispute`, { by: session.role === 'worker' ? 'worker' : 'employer', note: 'Needs manual review before settlement.' }), 'Dispute opened')}>Open dispute</button>
      </div>
    </section>
    <${DetailPanel} job=${job} session=${session} />
  </div></div>`
